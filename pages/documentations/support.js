const SupportComponent = {
  data() {
    return {
      ...
      topics: [
        { id: 1, title: 'Erste Hilfe', icon: '🆘', content: [], subcategories: [] },
        { id: 2, title: 'FAQ', icon: '❓', content: [], subcategories: [] },
        { id: 3, title: 'Kontakt Support', icon: '📧', content: [], subcategories: [] },
        { id: 4, title: 'Troubleshooting', icon: '🔧', content: [], subcategories: [] }
      ],
      componentLibrary: [
  ...
  methods: {
    ...
    saveToLocalStorage() {
      localStorage.setItem('support_docs', JSON.stringify(this.topics))
    },
    loadFromLocalStorage() {
      const saved = localStorage.getItem('support_docs')
      if (saved) {
    ...
    <div class="min-h-screen bg-gray-50 flex">
      ...
      <div class="w-80 bg-white shadow-xl border-r border-gray-200 flex flex-col">
        <div class="p-4 border-b border-gray-200">
          <h2 class="text-2xl font-bold text-gray-800 mb-4">📚 Support</h2>
          ...
          <button 
            @click="addNewTopic"
            class="mt-3 w-full bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg text-sm font-semibold transition-colors">
            + Neues Thema
          </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-2">
          <div v-for="topic in filteredTopics" :key="topic.id" class="space-y-1">
            <div 
              ...
              :class="['p-3 rounded-lg cursor-pointer transition-all', activeTopicId === topic.id && !activeSubcategoryId ? 'bg-blue-50 border-2 border-blue-500' : 'hover:bg-gray-50 border-2 border-transparent']">
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  <span class="text-2xl mr-3">{{ topic.icon }}</span>
                  <span class="font-semibold text-gray-800">{{ topic.title }}</span>
                </div>
                <span v-if="topic.subcategories.length > 0" class="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
            ...
            <div v-if="activeTopicId === topic.id && topic.subcategories.length > 0" class="ml-6 space-y-1">
              <div 
                ...
                :class="['p-2 rounded-lg cursor-pointer transition-all text-sm', activeSubcategoryId === subcategory.id ? 'bg-blue-100 border-2 border-blue-400' : 'hover:bg-gray-100 border-2 border-transparent']">
                <div class="flex items-center justify-between">
            ...
            <!-- Add Subcategory Button (nur wenn Thema aktiv und Edit Mode) -->
            <div v-if="activeTopicId === topic.id && editMode" class="ml-6">
              <button 
                @click.stop="addSubcategory"
                class="w-full text-left p-2 text-sm text-blue-600 hover:bg-blue-50 rounded-lg">
      ...
      <div class="flex-1 overflow-y-auto">
        <div class="max-w-4xl mx-auto p-8">
          ...
          <div v-if="editMode" class="bg-blue-50 border-2 border-blue-200 rounded-lg p-4 mb-6">
            ...
            <div class="flex flex-wrap gap-2">
              <button 
                ...
                class="bg-white hover:bg-blue-100 border-2 border-blue-300 px-4 py-2 rounded-lg font-semibold text-sm transition-colors">
          ...
          <div class="space-y-4">
            ...
            <div v-for="component in activeContent" :key="component.id" class="relative group">
              ...
              <!-- Card -->
              <div v-else-if="component.type === 'card'" class="bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl shadow-lg p-6">
